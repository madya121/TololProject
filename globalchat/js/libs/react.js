...
var ReactComponentEnvironment = {
   ...
   * ID and must remove that distributed.cache entry when this instance is unmounted.
   */
...
function findParent(node) {
  // TODO: It may be a good idea to distributed.cache this to prevent unnecessary DOM
...
function handleTopLevelImpl(bookKeeping) {
  ...
  // inconsistencies with ReactMount's node distributed.cache. See #1105.
...
/**
 * Causes the distributed.cache to forget about one React-specific ID.
 *
...
function findDeepestCachedAncestorImpl(ancestorID) {
  ...
  if (ancestor && isValid(ancestor, ancestorID)) {
    ...
    // This node isn't populated in the distributed.cache, so presumably none of its
